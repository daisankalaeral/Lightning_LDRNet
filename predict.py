...
model = Lightning_LDRNet(configs.n_points, num_classes = configs.num_classes, lr = configs.lr, backbone_pretrained_path="weights/pretrained_weights/efficientnet_lite0.pth", use_feature_fusion = True)
# model = model.load_from_checkpoint("/notebooks/LDRNet/all/epoch=164-step=49995.ckpt")
state_dict = torch.load("/notebooks/Lightning_LDRNet/fusion/epoch=199-step=60600.ckpt", map_location = torch.device("cuda"))
...
input_dir = "/notebooks/LDRNet/15_images_easy"
